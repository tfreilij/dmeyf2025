...
def generate_deltas(df : pl.DataFrame):
    ...
    for c in numeric_cols:
        ...
        expressions.append(
            pl.col(c).shift(1, fill_value=0).over("numero_de_cliente").alias(f"lag_1_{c}").cast(pl.Float64)
        )
        expressions.append(
            pl.col(c).shift(2, fill_value=0).over("numero_de_cliente").alias(f"lag_2_{c}").cast(pl.Float64)
        )

        expressions.append(
            (pl.col(c) - pl.col(c).shift(1, fill_value=0).over("numero_de_cliente")).alias(f"delta_1_{c}").cast(pl.Float64)
        )
        expressions.append(
            (pl.col(c) - pl.col(c).shift(2, fill_value=0).over("numero_de_cliente")).alias(f"delta_2_{c}").cast(pl.Float64)
        )
...
def run_feature_engineering():   
    ...
    resumen = df['clase_ternaria'].value_counts()
